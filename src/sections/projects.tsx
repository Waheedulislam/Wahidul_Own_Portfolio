...
const filters = ["All", "Frontend", "Backend", "Full Stack", "API", "Dashboard"] as const;

function CaseStudyField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
    ...
    <Card interactive className="overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-4 p-6">
        <div>
          ...
          <h3 className="mt-3.5 font-display text-xl font-semibold">{project.name}</h3>
        ...
        <div className="flex gap-2">
          {project.liveUrl && (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer">
              <Button variant="outline" size="sm"><ExternalLink size={14} />Live Demo</Button>
            </a>
          )}
          {project.githubUrl && (
            <a href={project.githubUrl} target="_blank" rel="noopener noreferrer">
              <Button variant="outline" size="sm"><Github size={14} />Code</Button>
            </a>
          )}
        </div>
      </div>
      ...
      {open && (
        <div className="grid gap-6 border-t border-border bg-foreground/[0.015] p-6 sm:grid-cols-2">
          <CaseStudyField label="Problem Statement">{project.problem}</CaseStudyField>
          ...
          <CaseStudyField label="Tech Stack">{project.techStack.join(", ")}</CaseStudyField>
          ...
          <CaseStudyField label="Challenges Faced">{project.challenges}</CaseStudyField>
          <CaseStudyField label="Lessons Learned">{project.lessonsLearned}</CaseStudyField>
        </div>
      )}
    </Card>
  );
      ...
      <div className="container-px">
        <div className="mb-12 max-w-[640px]">
          ...
          <p className="mt-3.5 max-w-[56ch] text-muted-foreground">
            Real systems, broken down the way I&apos;d walk a hiring manager through them.
          </p>
        </div>
        ...
        {visible.length === 0 ? (
          ...
          >
            ...
            <p className="text-[15px] font-semibold">Project case studies go here</p>
            <p className="max-w-[44ch] text-sm text-muted-foreground">
              Add entries to <code className="font-mono text-accent">src/data/projects.ts</code>{" "}
              with real problem statements, architecture and stack for each project — nothing is
              auto-generated here.
            </p>
          </motion.div>
        ) : (
